...
class ZohoSmartPollingProcessor:
    ...
    def save_reply_as_draft(self, original_email, reply_body):
        """Save reply as draft in Drafts folder and mark email as read"""
        try:
            reply = MIMEMultipart()
            ...
            mail.select('INBOX')
            mail.store(original_email['id'], '+FLAGS', '\\Seen')
            
            mail.close()
            mail.logout()
            
            logger.info(f"Draft reply saved and email marked as read for {original_email['sender']}")
            return True
            
        except Exception as e:
            logger.error(f"Error saving draft or marking as read: {e}")
            return False
